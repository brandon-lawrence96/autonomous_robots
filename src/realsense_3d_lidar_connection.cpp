...
// Creating 4 objects that contain pointer types with PointXYZI as parameters
...
// This first loop will run 6 times before leaving this function
{
  if (!systemInited) {
    systemInitCount++;
...
void posecallback(const nav_msgs::Odometry::ConstPtr& data)
{
    ...
    ros::NodeHandle nh;
    ros::Publisher pubOdom = nh.advertise<nav_msgs::Odometry>("/state_estimation", 10);
    ...
    float vehicleRecRoll = roll;
    float vehicleRecPitch = pitch;
    float vehicleRecZ = odom.pose.pose.position.z;

    vehicleRoll = terrainRoll * cos(yaw) + terrainPitch * sin(yaw);
    vehiclePitch = -terrainRoll * sin(yaw) + terrainPitch * cos(yaw);
    yaw += 0.005 * vehicleYawRate;
    if (yaw > PI) yaw -= 2 * PI;
    else if (yaw < -PI) yaw += 2 * PI;

    vehicleX += 0.005 * cos(yaw) * vehicleSpeed 
              + 0.005 * vehicleYawRate * (-sin(yaw) * sensorOffsetX - cos(yaw) * sensorOffsetY);
    vehicleY += 0.005 * sin(yaw) * vehicleSpeed 
              + 0.005 * vehicleYawRate * (cos(yaw) * sensorOffsetX - sin(yaw) * sensorOffsetY);
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subOdom = nh.subscribe<nav_msgs::Odometry> ("/t265/odom/sample", 10, posecallback);
